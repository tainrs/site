// Check if JQuery is loaded and '$' is defined
if (typeof jQuery != 'undefined') {
    // jQuery is loaded => print the version
    console.log('jQuery ' + jQuery.fn.jquery + ' is loaded');

    jQuery(function loadJSON() {
        var image = document.getElementsByTagName("h1")[0].innerHTML;

        // Check if image is a valid image name (no spaces, only lowercase alphanumeric characters and dashes)
        if (!/^[^\s]+\/[^\s]+$/.test(image)) {
            return;
        }

        console.log('Loading JSON for ' + image);

        const splitImage = image.split("/");

        document.getElementById("github-link")?.href && (document.getElementById("github-link").href = "https://github.com/" + image);
        document.getElementById("github-link")?.href && (document.getElementById("dockerio-link").href = "https://hub.docker.com/r/" + image);
        // document.getElementById("ghcrio-link").href="https://github.com/orgs/" + splitImage[0] + "/packages/container/package/" + splitImage[1];

        // Supress JQuery 404 error
        $.getJSON('https://raw.githubusercontent.com/' + image + '/main/links.json', data => {
            ...
            console.log('Error loading links.json')
        });

        $.getJSON('https://raw.githubusercontent.com/' + image + '/main/tags.json',
            data => {
                ...
                console.log('Error loading tags.json')
            });
    });
}
