...
upstream_digest=$(jq -re '.manifests[] | select (.platform.architecture == "amd64" and .platform.os == "linux").digest' <<< "${manifest}")
...
jq --sort-keys \
    ...
    '.upstream_digest = $upstream_digest' <<< "${json}" | tee VERSION.json
